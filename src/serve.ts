 ...
 *
 * Builds the {@link Orchestrator} (stub/fake by default; `--real` opts into the token-spending
 * harness + `gh`/`git` adapter) and serves the HTTP command API + the SSE live stream. This is the
