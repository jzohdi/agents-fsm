...
- ~~Auto-merging approved PRs.~~ Shipped as an **opt-in per-repo `auto_merge` flag** (default **off**, [#15](https://github.com/jzohdi/agents-fsm/issues/15)): with it off, `done` means merge-ready and a human clicks merge (unchanged); with it on, a `done` run merges its own PR into base, gated on exactly the same approval `done` already requires (no new bypass).
...
| — | `done` | PR approved and merge-ready. A human performs the merge — **unless** the repo opts into `auto_merge` ([#15](https://github.com/jzohdi/agents-fsm/issues/15)), in which case the run merges its own PR into base on the same approval signal. |
...
- **No stacked PRs: dependent work never starts against unmerged work.** A deliberate simplification. When issue B depends on issue A, B's run is not admitted until A's PR is *merged into the base branch*; B then branches off the up-to-date base. We never base one open PR on another open PR. This keeps git history linear, rebases trivial, and makes "dependency satisfied" a single hard signal (A is merged) instead of a fragile chain of in-flight branches. The accepted cost: unless the repo opts into `auto_merge` ([#15](https://github.com/jzohdi/agents-fsm/issues/15)), dependent work waits for a human to merge the dependency — on purpose, not by oversight. (With auto-merge on, the approved run merges its own PR, so the dependency signal fires without a human.)
...
- **Base-branch discipline — no stacked PRs (§3.1):** a dependent run is admitted only after its dependencies are *merged* to the base branch; it then branches off up-to-date code. Dependent work never starts against an unmerged branch — a firm invariant, not just a convenience. (Consequence: unless the repo opts into `auto_merge` (#15), dependent runs wait for a human to merge the dependency.)
...
**Sequencing & safety:** serial by default (in-flight cap 1) means it needs **no concurrency machinery** — it can ship on the serial loop ahead of Milestone 8 Phase B, with parallel pickup (cap > 1) folding in later once worktrees exist. By default it never auto-merges (a human still reviews and merges every PR — the same no-stacked-PRs discipline as §3.1), so "continuous" means *continuous pickup*, not unattended shipping; a repo that also opts into `auto_merge` (#15) closes that last gap for its own approved PRs. A run that escalates to `needs_human` holds its slot until resolved, so a broken issue pauses the queue rather than silently skipping ahead.
...
`auto_merge` ([#15](https://github.com/jzohdi/agents-fsm/issues/15)), in which case an approved run
