...
**Next: Milestone 4** (real agents). Write the per-stage system prompts and the per-phase prompts (self-review rubric, cleanup/simplify), set per-phase model defaults, and wire the real Claude Code subprocess executor + the `GitHub` adapter into the Agent Runner / Event Loop — including the working-tree lifecycle (prepare a tree per run, commit/push artifacts, open/update the PR idempotently). The integration seams (`StageExecutor`, `GitHub`) and their fakes are in place from Milestone 3, so this is wiring real prompts and the working-tree lifecycle onto existing interfaces — not a rework.
