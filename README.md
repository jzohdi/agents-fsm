...
**On top of Milestone 7 — resolved-lane archive + UI/CLI polish** (current `HEAD`: **415 tests passing, 1 skipped** — the flag-gated real-e2e; the figure now includes Milestone 8 Phase A + B1). Hardening and ergonomics that landed after the M7 core, not a new milestone:
...
**Added in Milestone 8 — multi-repo + parallel execution** (design in [plans/milestone-8.md](plans/milestone-8.md)). Phase A (multi-repo, serial) and Phase B B1 (the worker pool) have landed; B2 (per-run worktrees / concurrent crash-recovery) and B3 (rate-limit handling + optional global cost ceiling) are next. The pure FSM engine and its tests are untouched, as the milestone requires.
